<?php
echo '<h2>' .  $player['FullName'] . ' ' . $year . ' Game Log</h2>';
...
if ($playerSeason['PassAtt'] > 0 || $played > 0) {
    ...
    foreach ($statsService as $player) {
        if ($player['PlayerID'] == $_GET['player'] && $player['Season'] == '1985' && ($player['WeekNumber'] == 0 || $player['WeekNumber'] == 17)) {
    ...
    foreach ($statsServiceTotal as $player) {
        if ($player['PlayerID'] == $_GET['player'] && $player['Season'] == $year) {
            echo '<tr>' .
            '<th>TOTAL</th><th>' . 
            '</th><th>' . 
            $wins . ' - ' . $losses .'</th><th>' .
