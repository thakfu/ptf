...
if ($_SESSION['admin'] !== '2') {
    ...
    foreach ($allOffered as $ao) {
        ...
        $faServiceu = faPlayerService('offer', 'all');

        usort($faServiceu, fn($a, $b) => $a['LastName'] <=> $b['LastName']);

        $finalofferu = array();
        $playersu = array();
        foreach ($faServiceu as $fas) {
            if ($fas['result'] == 2) {
                array_push($playersu, $fas['PlayerID']);
            }
        }
        $allOfferedu = (array_unique($playersu));

        foreach ($allOfferedu as $ao) {
            echo '<table border="1">';
            echo '<tr>
            <th>Player</th>
            <th>ID</th>
            ...
            foreach ($faServiceu as $fas) {
                $rand = rand(1,50);
                if ($fas['result'] == 2 && $ao == $fas['PlayerID']) {
                    if ($rand > $hiRand) {
                        $hiRand = $rand;
                        $hiTeam = $fas['TeamName'];
                        $hiTeamID = $fas['TeamID'];
                    ...
                    <td>' . $fas['TeamName'] . '</td>
                    <td>500,000</td>
                    <td style="background-color:yellow"><b>' . $rand . '</b></td>
                    </tr>';
                }
            }
            echo '</table>';
            echo 'WINNER: <b>' . $hiTeam . '</b><br><br>';
            array_push($offeru, $hiOffer, $ao, $hiTeamID);
            array_push($finalofferu, $offeru);
        }
}
